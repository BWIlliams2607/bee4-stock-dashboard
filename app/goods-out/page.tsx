...
type GoodsOutLog = {
  id: number; timestamp: string; barcode: string;
  name: string; sku: string; quantity: number;
  location: string; shelf: string
}
...
export default function GoodsOutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!selected || !quantity || !location || !shelf) return toast.error("Please fill all fields")
    const payload = { barcode:selected.barcode, name:selected.name, sku:selected.barcode,
                      quantity:Number(quantity), location, shelf }
    const res = await fetch("/api/goods-out",{method:"POST",
  ...
  return (
    <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="space-y-8">
      ...
      <form onSubmit={handleSubmit} className="grid max-w-3xl mx-auto gap-6 bg-muted/70 p-6 rounded-2xl shadow-xl grid-cols-1 md:grid-cols-3">
        <div className="col-span-1 md:col-span-3">
          ...
          <div className="relative">
            <button type="button" onClick={()=>setScannerOpen(true)} className="absolute left-2 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground" title="Scan barcode">
              <Camera size={20}/>
            </button>
            <Combobox value={selected} onChange={setSelected}>
              <Combobox.Input ref={comboRef}
                ...
                <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-lg bg-background py-1 text-sm shadow-lg z-10">
                  {filtered.map(p=>(
                    <Combobox.Option key={p.id} value={p}
                      className={({active})=>`cursor-pointer px-4 py-2 ${active?"bg-rose-500 text-white":""}`}>
        ...
        <div>
          ...
          <input type="number" min={1} required value={quantity} onChange={e=>setQuantity(e.target.value)}
            className="w-full rounded-lg border border-border px-4 py-2 bg-background" placeholder="e.g. 25"/>
        </div>
        <div>
          <label className="block text-sm font-semibold mb-1">Location</label>
          <input required value={location} onChange={e=>setLocation(e.target.value)}
            className="w-full rounded-lg border border-border px-4 py-2 bg-background" placeholder="e.g. Warehouse 1"/>
        </div>
        <div>
          <label className="block text-sm font-semibold mb-1">Shelf</label>
          <input required value={shelf} onChange={e=>setShelf(e.target.value)}
            className="w-full rounded-lg border border-border px-4 py-2 bg-background" placeholder="e.g. A1, B3"/>
        </div>
        <div className="md:col-span-3">
          <Button type="submit" className="w-full flex justify-center gap-2">Remove Stock <CheckCircle size={20}/></Button>
        </div>
      </form>

      {scannerOpen && (
        <CameraBarcodeScanner onDetected={code=>{
      ...
      <div className="max-w-3xl mx-auto space-y-4">
        <input type="text" value={searchLog} onChange={e=>setSearchLog(e.target.value)}
          placeholder="Search logs…" className="w-full rounded-lg border border-border px-4 py-2 bg-background text-sm"/>

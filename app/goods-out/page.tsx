...
export default function GoodsOutPage() {
  ...
  useEffect(() => {
    fetch("/api/goods-out")
      ...
      .then((data: GoodsOutLog[]) => setLog(data))
      .catch(() => toast.error("Could not load Goods Out log"))
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { barcode, name, sku, quantity: parseInt(quantity, 10), location, shelf }
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-12">
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto bg-muted/70 shadow-xl rounded-2xl p-6 md:p-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <input required type="number" min={1} value={quantity} onChange={e => setQuantity(e.target.value)} placeholder="e.g. 25" className="w-full rounded-lg border border-border px-4 py-2 bg-background text-foreground" />
        </div>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="overflow-x-auto rounded-lg bg-muted/70 shadow-xl p-4">
          <table className="min-w-full text-sm">
            <thead><tr className="border-b border-border">{["Time","Barcode","Product","Qty","Location","Shelf"].map(h=><th key={h} className="p-2 text-left">{h}</th>)}</tr></thead>
            <tbody>
              {log.length===0?(

...
type IncomingStockLog = {
  ...
  name: string; sku: string; expectedDate: string;
...
export default function IncomingStockPage() {
  ...
  const handleSubmit = async (e:React.FormEvent)=>{
    e.preventDefault()
    if(!selected||!expectedDate||!quantity||!supplier) return toast.error("Please fill all fields")
    const payload={barcode:selected.barcode,name:selected.name,sku:selected.barcode,
                   expectedDate,quantity:Number(quantity),supplier}
    const res=await fetch("/api/incoming-stock",{method:"POST",
    ...
    <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="space-y-8">
      ...
      <form onSubmit={handleSubmit} className="grid max-w-3xl mx-auto gap-6 bg-muted/70 p-6 rounded-2xl shadow-xl grid-cols-1 md:grid-cols-3">
        ...
        <div>
          ...
          <input type="date" required value={expectedDate} onChange={e=>setExpectedDate(e.target.value)}
        ...
        <div>
          ...
          <input type="number" min={1} required value={quantity} onChange={e=>setQuantity(e.target.value)}
            className="w-full rounded-lg border border-border px-4 py-2 bg-background" placeholder="e.g. 50"/>
        </div>
        ...
        <div>
          ...
          <input required value={supplier} onChange={e=>setSupplier(e.target.value)}
            className="w-full rounded-lg border border-border px-4 py-2 bg-background" placeholder="e.g. Acme Inc."/>
        </div>

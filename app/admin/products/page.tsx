...
export default function ProductAdminPage() {
  ...
  const [newProdSupplierId, setNewProdSupplierId] = useState<number>();
  ...
  const handleAddProduct = async () => {
    ...
    setNewProdDesc("");
    setNewProdCats([]);
    setNewProdSupplierId(undefined);
    setNewProdLocationId(undefined);
    setNewProdShelfId(undefined);
  ...
  const saveProduct = async (payload: EditPayload) => {
        ...
        <section className="bg-gray-800 shadow-lg rounded-2xl p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            ...
            <div>
              ...
              <select
                value={newProdLocationId ?? ""}
                onChange={(e) =>
                  setNewProdLocationId(
                    e.target.value ? +e.target.value : undefined
                  )
                }
                className="w-full h-12 rounded-lg border border-gray-700 bg-gray-700 px-4 text-white"
              ...
              <tbody>
                {filteredProds.map((p) => (
                  ...
                  >
                    ...
                    <td className="p-3">{p.defaultSupplier?.name || "—"}</td>
                    <td className="p-3">{p.defaultLocation?.name || "—"}</td>
                    <td className="p-3">{p.defaultShelf?.name || "—"}</td>
